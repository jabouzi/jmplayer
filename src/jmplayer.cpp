...
void Jmplayer::initUi(){
    ...
    add_button->setIcon(style()->standardIcon(QStyle::SP_DialogOpenButton));
...
void Jmplayer::loadFirstTime()
{
    filesList = QFileDialog::getOpenFileNames(this,tr("Open File"),".",tr("audios et videos (*.mp3 *.wma *.ogg *.wave *.midi *.mp4 *.flv *.ogv *.mpeg *.mpg *.avi *.divx *.wmv *.mov)"));
...
void Jmplayer::getFilesInfosToAppend(QStringList files,QStringList files2)
{
    for (int i = 0; i < files.size(); ++i) {
        QStringList templist = files.at(i).split("/");
        QString filename = templist[templist.size()-1];
        QStringList templist2 = files2.at(i).split("/");
        QString filename2 = templist2[templist2.size()-1];
        QString systemArg ="mplayer -identify -frames 0 "+files.at(i)+" 2>/dev/null > /tmp/"+filename+".log";
        system(systemArg.toLatin1().data());    
        setInfos("/tmp/"+filename2+".log");
        QString s = "/tmp/"+filename+".log";        
        filesList.append(files.at(i));
        filesList2.append(files2.at(i));
    }
}
